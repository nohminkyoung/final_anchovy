...
def index(request):
    ...
    status_dic = {'goal' : 0, 'percent':0, 'lv':0, 'name':''}
    ...
    target_training = Train.objects.filter(username=login_user).aggregate(Sum('train_accurate_count')) # training 값은 단일 값이 아니기 때문에 filter 사용
    # 마지막 운동 기록
    target_recent = Train.objects.filter(username=login_user).last() 
    ...
    max_value = status_dic['goal']
    status_dic['percent'] = int(round(user_value/max_value, 2) * 100)

    context = {'log_user':target_user,'log_status':target_status, 'log_train': target_training, 'log_dic': status_dic, 'log_recent':recent_record}
