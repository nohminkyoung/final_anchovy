        ...
        try: # 사용자 중 아이디가 존재하는지
            ...
            # 친구 추가 할 수 있음
                ...
                status_data = Friend(username=user.username, friend_name=fd_id.username, 
                                    friend_nickname = fd_id.nickname, friend_protein=fd_id.protein, friend_lv=fd_id.character_lv , author_id = user.author_id, fd_id = fd_id.author_id) # friend_lv 추가하기 friend_lv=fd_id.character_lv
                status_data.save()
                return HttpResponse(json.dumps({'status':status}))
